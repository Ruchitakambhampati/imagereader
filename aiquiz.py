...
# Load SentenceTransformer with device placement
similarity_model = SentenceTransformer("all-MiniLM-L6-v2", device=str(device))
            ...
            try:
                ...
                if questions:
                    ...
                    if st.button("Are these questions okay?"):
                        if st.button("Yes"):
                            if st.button("Can I create a form?"):
                                formspree_endpoint = f"https://formspree.io/f/{form_owner_email}"  # Replace with actual Formspree URL
                                form_html = f"""
                                <html>
                                <head><title>Quiz Form</title></head>
                                <body>
